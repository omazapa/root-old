...
void type( int argc=0, char *argv[]=0 )
...
//
  point_type = ROOT::Mpi::INT.Create_contiguous ( 3 );
  point_type.Commit ( );
  ...
  if ( id == 0 ) 
  {
    ...
    ROOT::Mpi::COMM_WORLD.Send ( &point, 1, point_type, dest, tag );
    ...
    source = 1;
    tag = 2;
    ROOT::Mpi::COMM_WORLD.Recv ( &point, 1, point_type, source, tag,
      status );
  ...
  else if ( id == 1 )
  {
    ...
    ROOT::Mpi::COMM_WORLD.Recv ( &point, 1, point_type, source, tag, status );
