...
namespace ROOT {
   namespace Mpi {
      ...
      // null handles
      const TGroup         GROUP_NULL = MPI::GROUP_NULL;
      const TWin           WIN_NULL = MPI::WIN_NULL;
      const TInfo          INFO_NULL = MPI::INFO_NULL;
      TIntracomm           COMM_NULL(MPI::COMM_NULL);
      const TDatatype      DATATYPE_NULL = MPI::DATATYPE_NULL;
      TRequest             REQUEST_NULL = MPI::REQUEST_NULL;
      const TOp            OP_NULL = MPI::OP_NULL;
      const TErrhandler    ERRHANDLER_NULL = MPI::ERRHANDLER_NULL;
      const TFile          FILE_NULL = MPI::FILE_NULL;

      // constants specifying empty or ignored input
      const Char_t**       ARGV_NULL = NULL;
      const Char_t***      ARGVS_NULL = NULL;

      // empty group
      const TGroup  GROUP_EMPTY = MPI::GROUP_NULL;
